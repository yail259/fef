...
public class FirstOrderSubscription extends FirstOrder implements SubscriptionOrder{

    private int numShipments;

    public FirstOrderSubscription(int id, LocalDateTime date, double discountRate, int discountThreshold, int customerID, int numShipments) {
    ...
    public Order copy() {
        ...
        Order copy = new FirstOrderSubscription(getOrderID(), getOrderDate(), getDiscountRate(), getDiscountThreshold(), getCustomer(), numShipments);
